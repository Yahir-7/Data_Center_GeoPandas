...
# Extract county FIPS from Census GEO_ID values.
...
# Load county boundaries as the base dataset so the output stays county-level.
...
# Add extreme heat data from EJScreen and average it to county level.
...
# Add drought severity data and average tract values to county level.
...
# Add nighttime light pollution data and average it to county level.
...
if "county_ID" in alan.columns:
    ...
    raise ValueError("Could not find county_ID or GEOID in ALAN file.")
...
# Add PLACES health data using adult population-weighted county averages.
...
output_file = BASE_DIR / "geopandas_census-datasets.csv"
